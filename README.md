...
| 阶段 | 目标 | 恐怖/玩法元素 |
|---|---|---|
| **Level 0** | 找齐 3 枚保险丝 → 东端 EXIT 门解锁 → 逃入泳池层 | 灯管随机骤熄、低频嗡鸣、远处吱嘎声；**黑影实体**（红眼剪影）在拾取保险丝后必定现身，靠近触发惊吓并被拖回出生点 |
| **Level 37** | 自由探索标准游泳馆，集齐 6 个漂浮物（小黄鸭 ×4 / 救生圈 ×2） | 潜入深水触发水下折射滤镜；集齐后弹出 **通关结算**（用时 / 惊吓次数 / 收集度），支持一键**再玩一次** |
